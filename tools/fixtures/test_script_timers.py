#!/usr/bin/env python3
"""Exercise a saved script timer whose handler omits RETURN 1."""
...
from make_fixture import SCRIPT_TIMER_ITEM_UID
from run_suite import shutdown_failures


ACCOUNT_NAME = "ScriptTimerListener"
LOGIN_VALUE = "script-timer-pw"
TRIGGER_MARKER = "SPHERE_SCRIPT_TIMER_TRIGGERED"
REMOVED_MARKER = "SPHERE_SCRIPT_TIMER_REMOVED"
TIMER_ERROR = "Timer expired without DECAY flag 'synthetic script timer'?"
...
def main() -> int:
    ...
    def exercise() -> None:
        ...
        if sock is None:
            raise RuntimeError("script timer probe did not reach the character list")
        try:
            sock.sendall(
                make_char_create(
                    name=ACCOUNT_NAME,
            ...
            if not response or find_start_packet(decode_game_response(response)) is None:
                raise RuntimeError("script timer probe did not enter the world")
    ...
    if trigger_values != [1]:
        failures.append(f"script timer callback marker values were {trigger_values!r}")
    if removed_values != [0]:
        failures.append(f"script timer removal marker values were {removed_values!r}")
    if log_contents.count(TIMER_ERROR):
        failures.append(
            f"script timer still emitted {log_contents.count(TIMER_ERROR)} generic timer error(s)"
        )
    if "world load: created_items=1 created_chars=1" not in log_contents:
        failures.append("saved script timer fixture did not load its item and owner")
    if failures:
        print("script-timers probe failed:", file=sys.stderr)
        for failure in failures:
            print(f"- {failure}", file=sys.stderr)
        print(f"- expected item uid={SCRIPT_TIMER_ITEM_UID:#x}", file=sys.stderr)
        ...
        return 1
    print(
        "script-timers probe passed: callback/removal markers once, "
