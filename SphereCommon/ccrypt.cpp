...
const BYTE CCrypt::sm_seed_table[2][CRYPT_GAMEKEY_COUNT][2][CRYPT_GAMESEED_LENGTH] =
{
	...
	0xF8, 0xC4, 0xD8, 0x72, 0x54, 0xFC, 0xF9, 0xDE,	// first key
...
BYTE CCrypt::EncryptByte(BYTE data)
{
	BYTE result;
	if ( GetCryptVer() < 0x200040 )
	{
		if(!m_gameBlockPos)
		{
		...
		m_gameBlockPos--;
		result = data ^ m_Key.u_cKey[m_gameBlockPos];

		m_Key.u_cKey[m_gameBlockPos] = result;
	}
	if ( GetCryptVer() >= 0x200003 )
	{
		result = m_NewCoder.CodeNewByte(result);
	}
	return result;
}

BYTE CCrypt::DecryptByte(BYTE data)
{
	if ( GetCryptVer() >= 0x200003 )
	{
		data = m_NewCoder.CodeNewByte(data);
		if ( GetCryptVer() >= 0x200040 )
		{
			return data;
		}
	...
	m_gameBlockPos--;
	BYTE result = data ^ m_Key.u_cKey[m_gameBlockPos];
	m_Key.u_cKey[m_gameBlockPos] = data;
	return result;
}
...
void CCrypt::Decrypt( BYTE* pOutput, const BYTE* pInput, int iLen )
{
	...
	if ( m_ConnectType == CONNECT_LOGIN || GetCryptVer() < 0x126000 )
	{
		// Basic de-crypt is for login, and old clients.
		CCryptBase::Decrypt( pOutput, pInput, iLen );
		return;
	}

	if ( GetCryptVer() < 0x200040 && m_gameStreamPos + iLen > CRYPT_GAMETABLE_TRIGGER)
	{
	...
	{
		pOutput[i] = DecryptByte( pInput[i] );
	}
...
void CCrypt::Encrypt( BYTE * pOutput, const BYTE * pInput, int iLen )
{
	...
	for ( int i=0; i<iLen; i++ )
	{
		pOutput[i] = EncryptByte( pInput[i] );
	}

	m_gameStreamPos += iLen;
}

void CCrypt::PrepareKey( CCryptKey & key, int iTable )	// static
{
	const DWORD *pCodes = sm_dwCodingData[iTable];
	bool fUnknown = true;
	if(fUnknown)		// ??? seems to ALWAYS be TRUE ???
	{
	...
	{
		key.u_iKey[1] ^= pCodes[17];
		for(int i=8; i--; )
		{
			key.u_iKey[0] ^= pCodes[i*2+1]^(((pCodes[18+key.u_cKey[7]]+pCodes[18+key.u_cKey[6]+0x100])^pCodes[18+key.u_cKey[5]+0x200])+pCodes[18+key.u_cKey[4]+0x300]);
			key.u_iKey[1] ^= pCodes[i*2+2]^(((pCodes[18+key.u_cKey[3]]+pCodes[18+key.u_cKey[2]+0x100])^pCodes[18+key.u_cKey[1]+0x200])+pCodes[18+key.u_cKey[0]+0x300]);
		}
		key.u_iKey[0] ^= pCodes[0];
	}
...
void CCrypt::InitCrypt()
{
	// recalc crypt based on current params.
	if ( m_ConnectType != CONNECT_LOGIN )
	{
		...
		if ( GetCryptVer() >= 0x200003 )
		{
			m_NewCoder.InitCrypt( GetCryptSeed());
		}
	}

	CCryptBase::InitCrypt();
}

void CCrypt::SetCryptType( DWORD dwIP, CONNECT_TYPE type )
{
	m_ConnectType = type;
	SetCryptSeed( dwIP );
}
