...

{
	...
	CItemPtr pMeat = ContentFind( IT_MEAT_RAW );
...

	{
		...
		Use_EatQty( pMeat, MIN( iAmount - m_StatFood, iBite ));
...

	{
		...
		CItemPtr pItem = AreaItems.GetItem();
...

		{
			...
			CItemMemoryPtr pMemory = Memory_FindObj( pItem );
...

	{
		...
		CCharPtr pChar = AreaChars.GetChar();
...

{
...

	{
...

		{
			...
			Use_EatQty( pItem, MIN( iAmount - m_StatFood, iBite ));
...

{
	...
	CItemPtr pResBit = g_World.CheckNaturalResource( GetTopPoint(), IT_WATER, true );
...

	{
...

		{
			...
			int iEaten = pResBit->ConsumeAmount( MIN( m_StatFood, iBite ));
...

{
	...
	CItemPtr pResBit = g_World.CheckNaturalResource( GetTopPoint(), IT_GRASS, true );
...

	{
		...
		int iEaten = pResBit->ConsumeAmount( MIN( m_StatFood * 10, iBite ));
...

{
	...
	CItemPtr pFood = ContentFindType( IT_FOOD );
...

	{
		...
		Use_EatQty( pFood, MIN( iAmount - m_StatFood, iBite ));
...

	{
...

		{
...

			{
				...
				m_Act.m_Targ = pFood->GetUID();

				Skill_Cooking( CSkillDef::T_SUCCESS );
...

{
	...
	CItemContainerPtr pVendBox = GetBank( LAYER_VENDOR_EXTRA );

	CItemPtr pItem = pVendBox->ContentFindType(IT_FOOD);
...

	{
		...
		pItem->UnStackSplit( MIN( iAmount - m_StatFood, pItem->GetAmount()), this );
...
CItemPtr CChar::NPC_Food_Trade( bool fTest )

{
...

	{
		...
		CItemContainerPtr pBank = GetBank( (LAYER_TYPE) i );
...

{
	...
	ASSERT( m_pNPC.IsValidRefObj() );



	CCharPtr pFoodVendor;
...

	{
		...
		CCharPtr pTestChar = AreaChars.GetChar();
...

	{
		...
		CItemPtr pFood = pFoodVendor->NPC_Food_Trade( false );
...

		{
			...
			m_Act.m_pt = m_pNPC->m_Act_p_Prev;
...

	{
		...
		m_Act.m_pt = pFoodVendor->GetTopPoint();
...

{
	...
	TCHAR szFoodEntries[CSTRING_MAX_LEN];

	strcpylen( szFoodEntries, m_pDef->m_sFoodType, sizeof( szFoodEntries ));
...

	{
		...
		case 0: // Any
			...
			if ( m_pNPC->m_Brain == NPCBRAIN_VENDOR )
